...
/// The PullUpController (https://github.com/MarioIannotta/PullUpController)
open class PullUpController: UIViewController {
    ...
    /**
     ...
     There is therefore no need to add the fist one (pullUpControllerPreviewOffset), and/or the
     last one (pullUpControllerPreferredSize.height).
    ...
    /// Boolean if gestures is enabled
    ...
    /**
     ...
     - parameter completion: The closure to execute after the animation is completed. This block has no return value and
     takes no parameters. You may specify nil for this parameter.
     */
    ...
    /// Open the PullUpController
    open func open() {
        self.state = .opened
        self.pullUpControllerMoveToVisiblePoint(
            self.pullUpControllerPreferredSize.height,
            animated: true,
            completion: nil
        )
    }
    
    /// Close the PullUpController
    open func close() {
        self.state = .closed
        self.pullUpControllerMoveToVisiblePoint(
            self.pullUpControllerPreviewOffset,
            animated: true,
            completion: nil
        )
    }
    
    /// Push Up PullUpController
    open func pushUp(offset: CGFloat) {
        self.state = .pushed
        self.pullUpControllerMoveToVisiblePoint(
            offset + self.pullUpControllerPreferredSize.height,
            animated: true,
            completion: nil
        )
    }
    ...
    @objc fileprivate func handleInternalScrollViewPanGestureRecognizer(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        let shouldScrollingUpTriggerGestureRecognizer = !isScrollingDown
            && topConstraintValue != parentViewHeight - lastStickyPoint
...
extension UIViewController {
     ...
     */
    open func addPullUpController(_ pullUpController: PullUpController, animated: Bool = false) {
    ...
    open func removePullUpController(_ pullUpController: PullUpController, animated: Bool) {
        pullUpController.pullUpControllerMoveToVisiblePoint(0, animated: animated) {
            pullUpController.willMove(toParentViewController: self)
